	...
	public ArrayList<ThongTinDatThuePhong> timTatCaThongTinDatThuePhong() {
		try {
			...
			while (rs.next()) {
				...
				ThongTinDatThuePhong tt = new ThongTinDatThuePhong(maDatThue, kh, phong, ngayDatPhong, ngayNhanPhong,
						ngayTraPhong, loaiThue);
...
//			
		...
		if (dsThongTin.contains(tt)) {
			try {
				...
				String sql = "insert into ThongTinDatThuePhong values('" + tt.getMaDatPhong() + "','"
						+ tt.getKhachHang().getMaKH() + "','" + tt.getPhong().getMaPhong() + "','"
						+ tt.getNgayDatPhong() + "','" + tt.getNgayNhanPhong() + "','" + tt.getNgayTraPhong() + "','"
						+ tt.getLoaiThue().getMaLoaiThue() + "')";
	...
	public ThongTinDatThuePhong timThongTinDatThuePhongTheoMa(String maDatThue) {
		try {
			...
			while (rs.next()) {
				...
				Phong phong = new Phong(maPhong);
				LoaiThue loaiThue = new LoaiThue(maLoaiThue);
				tt = new ThongTinDatThuePhong(maDatThue, kh, phong, ngayDatPhong, ngayNhanPhong, ngayTraPhong,
						loaiThue);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ArrayList<ThongTinDatThuePhong> timThongTinDatThuePhongTheoMaKhachHang(String maKhachHang) {
		try {
			...
			while (rs.next()) {
				...
				LoaiThue loaiThue = new LoaiThue(maLoaiThue);
				tt = new ThongTinDatThuePhong(maDatThue, khachHang, phong, ngayDatPhong, ngayNhanPhong, ngayTraPhong,
						loaiThue);
				dsThongTin.add(tt);
			}
		} catch (Exception e) {
	...
	public ArrayList<ThongTinDatThuePhong> themDanhSachPhongDat(String maLoaiPhong, String trangThai, int soLuong,
		...
		for (Phong phong : dsPhong) {
			bdtp = new ThongTinDatThuePhong(maKH + 'i', new KhachHang(maKH), phong, ngayDat, ngayNhan, ngayTra,
					new LoaiThue(maLoaiThue));
			ds.add(bdtp);
			i++;
		}
	...
	// DAT NHIỀU PHÒNG
	public boolean datNhieuPhong(ArrayList<ThongTinDatThuePhong> dsDatThuePhong) {
		ConnectDB.getInstance();
		Connection con = ConnectDB.getConnection();
		Statement stmt = null;
			...
			if (dsDatThuePhong.contains(bangDatThuePhong)) {
				try {
					...
					String sql = "insert into BangDatThuePhong values('" + bangDatThuePhong.getMaDatPhong() + "','"
							+ bangDatThuePhong.getKhachHang().getMaKH() + "','"
							+ bangDatThuePhong.getPhong().getMaPhong() + "','" + bangDatThuePhong.getNgayDatPhong()
							+ "','" + bangDatThuePhong.getNgayNhanPhong() + "','" + bangDatThuePhong.getNgayTraPhong()
							+ "','" + bangDatThuePhong.getLoaiThue().getMaLoaiThue() + "',')";
					String sqls = "update Phong set trangThai = 'Đã đặt' where maPhong = '"
							+ bangDatThuePhong.getPhong().getMaPhong() + "'";
					n = stmt.executeUpdate(sql);
	...
	public ArrayList<ThongTinDatThuePhong> themDanhSachPhongThue(String maLoaiPhong, String trangThai, int soLuong,
		...
		int i = 0;
		for (Phong phong : dsPhong) {
			bdtp = new ThongTinDatThuePhong(maKH + 'i', new KhachHang(maKH), phong, null, ngayNhan, ngayTra,
					new LoaiThue(maLoaiThue));
			ds.add(bdtp);
			i++;
		}
...
// tính tổng tiền dịch vụ
	public double tinhTongTienDichVu(String maDatPhong) {
		...
		try {
			...
			String sqls =
					...
					"SELECT dp.maDVPhong, dp.maPhongThue, SUM(dp.soLuong * dv.GiaThue) AS TongTien FROM DichVuPhong dp JOIN DichVu dv ON dp.maDV = dv.maDV WHERE dp.maDatPhong = '"+ maDatPhong + "' GROUP BY dp.maDVPhong, dp.maPhongThue;";
	...
	public double tinhTongTienPhong(String maKhachHang) {
		ConnectDB.getInstance();
		Connection con = ConnectDB.getConnection();
		Statement stmt = null;
		double tongTien = 0;
		try {
			stmt = con.createStatement();
			String sql = "SELECT dp.maLoaiThue, dp.maPhongThue, SUM(dp.soLuong * dv.GiaThue) AS TongTien FROM LoaiThue dp JOIN ThongTinPhongDatThue dv ON dp.maLoaiThue = dv.maLoaiThue WHERE dp.khachHang = '"+ maKhachHang + "' GROUP BY dp.khachHang, dp.maPhongThue;";
	...
	// tính tổng tiền
	public double tinhTongTien(String maDatPhong) {
		double tongTien = 0;
		tongTien = tinhTongTienDichVu(maDatPhong) + tinhTongTienPhong(maDatPhong);
		return tongTien;
	}
